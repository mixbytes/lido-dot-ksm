...
def test_submit_unwrap(lido, oracle_master, vKSM, accounts, wstKSM):
    distribute_initial_tokens(vKSM, lido, accounts)
    ...
    vKSM.approve(wstKSM, deposit // 2, {'from': accounts[0]})
    wstKSM.submit(deposit // 2, {'from': accounts[0]})

    assert lido.balanceOf(accounts[0]) == deposit
    assert lido.balanceOf(wstKSM) == deposit // 2
    assert wstKSM.balanceOf(accounts[0]) == deposit // 2

    wstKSM.approve(accounts[2], deposit // 2, {'from': accounts[0]})
    wstKSM.transferFrom(accounts[0], accounts[2], deposit // 2, {'from': accounts[2]})

    wstKSM.unwrap(deposit // 2, {'from': accounts[2]})
